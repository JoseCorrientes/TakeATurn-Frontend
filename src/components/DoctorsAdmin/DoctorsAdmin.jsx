...
import { clearLoginDoctorError, sendDataLoginDoctor } from '../../actions/actions';
...
function LandingDoctors() {
  ...
  const navigate = useNavigate()
    ...
    //Este se fija al iniciar que el localstore tenga datos (va al dashboard), sino lo manda al login
    //   useEffect(()=>{
    //     let data=JSON.parse(localStorage.getItem('userOdonto'));
    //     console.log( ' data en el use Effecto inicial')
    //     console.log(data)
    //     console.log(' -------------------------')
    //     if (data!=null) navigate('/dashboard') 
    ...
    useEffect(()=>{
        if (loginDoctorValid) {
            ...
            window.localStorage.setItem('TATDoctorStringName', loginDoctorData.stringName )
    ...
    // useEffect(()=>{
    //         if (window.localStorage.getItem('TATDoctorEmail')) {
    //             console.log('hay algo en el localstora')
    //             console.log(window.localStorage.getItem('TATDoctorEmail'))
    //             navigate('/doctors/menu')
    //         }
    // },[])
  ...
  return (
    ...
    >
            ...
            >
                ...
                <div
                    className='flex flex-row justify-between w-3/4 h-[2rem] px-0 sm:w-1/3 mb-4 sm:h-1/6 sm:px-0 sm:mb-1  text-center '
                >
                <input 
                ...
                {(!emailValid || !passwordValid  ) && <button
                ...
                {
                visibleMessage && loginDoctorError=='2' && <p
                                        className='mt-4 px-4 sm:mr-4 font-Saira text-3xl text-white font-thin bg-red-600 text-center'
                >Usuario o Contraseña Invalidas.</p> 
                }
                {
                visibleMessage && loginDoctorError=='3' && <p
                className='mt-4 px-4 sm:mr-4 font-Saira text-3xl text-white font-thin bg-red-600 text-center'
                >Error de Servidor. Reintente.</p> 
                }
